...
impl<P> Hierarchy<P> {
    ...
    /// Get all children in a sub hierarchy as a `BitSet`
...
mod tests {
    ...
    fn test_all_children() {
        ...
        assert!(hierarchy.all_children(e1).iter().eq([e2.id()].iter().cloned()));
        assert_eq!(hierarchy.all_children(e2).iter().next(), None);
        assert!(hierarchy.all_children(e3).iter().eq([e4.id(), e5.id(), e6.id()].iter().cloned()));
        assert!(hierarchy.all_children(e4).iter().eq([e6.id()].iter().cloned()));
        assert_eq!(hierarchy.all_children(e5).iter().next(), None);
        assert_eq!(hierarchy.all_children(e6).iter().next(), None);
    }
}
